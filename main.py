...
def metadata(file, nb_lif):
    ...
    if file_extension == ".tif":
        ...
        # scale_x_px
        # from a string with '(a,b)' to numbers
        x_scale_px_1 = metadata_file["XResolution"].replace("(", "")
...
def real_mesh(mask, mask_inv, scale_x, scale_y, scale_z):
    ...
    # for the area: vtkMassProperties estimates the volume, the surface area, and the normalized shape index of a closed, triangle mesh.
    voxels = vol_inv_um.legosurface(vmin=0.5).triangulate()
...
def skeleton_3d_analysis(skeleton_3d, x_total, y_total, scale_x):
    ...
    return (
        sk_3d_projection_clean,
        branch_data,
        number_branches,
        number_isolated_branches,
        number_endpoint_junctions_branches,
        number_junction_junction_branches,
        number_circular_branches,
        number_junctions,
        number_endpoints,
...
def area_volume_diameters(
    self, surf, voxels, image, mask_skel, scale_x, scale_y, scale_z, x_total_um, y_total_um, depth, coord_src_0, coord_dst_0, coord_src_1, coord_dst_1
):
    ...
    # area of the piece studied under the microscope
    area_total_fibrin_gel = 2 * x_total_um * y_total_um + 2 * y_total_um * depth + 2 * x_total_um * depth
    ...
    # to label, we need the mask, the not-inverted one to have the holes between the vessels
    num = 0
    i = 0
    for slices in mask_skel:
    ...
    # Resize and mask
    # resize the slice to have access to the right measures
    slice_interest = cv2.resize(image[z_slice], dsize=(int(x_total_um), int(y_total_um)), interpolation=cv2.INTER_CUBIC)
    interest_blur = gaussian_filter(slice_interest, sigma=3)
    th = threshold_otsu(interest_blur)
    # inverted mask for filled vessels
    mask_interest_inv = np.array(interest_blur > th, dtype=int)
    # Medial axis and distance map
    sk_medial_axis, sk_distance_map = medial_axis(mask_interest_inv, return_distance=True)
    # we take the distance_map values only where the medial axis was computed
    x, y = sk_medial_axis.nonzero()
    diameter_horizontal = []
    for i in range(len(x)):
        # µm, as image resized above
        diameter_horizontal.append(2 * (sk_distance_map[x[i], y[i]]))

    # =================================   Diameter_vertical   ==================================================
    # The assumption made here is that the vertical diameter is measured at the center of the vessel
    #  (where we have the biggest height of the vessel).
    ...
    # The diameter of the vessel at a specific point can thus be calculated as the distance between
    #  the two intersection points between the vessel edges.
    ...
    for i in range(len(coord_src_0)):
        # middle of the skeleton branch
        middle_branch_point = [int((coord_src_0[i] + coord_dst_0[i]) / 2), int((coord_src_1[i] + coord_dst_1[i]) / 2)]
        # To be noticed: the middles are not exactly at the middle of each branch, due to curved branches.
        # the two intersection points
        pts = surf.intersect_with_line(
            p0=[middle_branch_point[0], middle_branch_point[1], 0], p1=[middle_branch_point[0], middle_branch_point[1], int(depth)], tol=0
        )

        # ensure not an infinite value from an opening of the surface mesh
        if len(pts) == 2:
    ...
    if len(diameter_vertical) == 0:
        self.message = messagebox.showerror(
            title="Error",
    ...
    return (
        area,
        volume,
        volume_total_fibrin_gel,
        area_total_fibrin_gel,
        percentage_occupancy,
        diameter_horizontal,
        diameter_vertical,
        mean_diameter_horizontal,
        mean_diameter_vertical,
    )
...
def max_projection(image, x_total, y_total, x_total_um, y_total_um):
    ...
    # max_proj to check on the skeleton in the pop-up window, and max_proj_to_plot in the display window
    return (max_proj, max_proj_toplot)
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        # empty row with minsize as spacing
        self.frame_left.grid_rowconfigure(18, minsize=10)
        self.label_1 = customtkinter.CTkLabel(master=self.frame_left, text="ƎDVAR", text_font=("Roboto Medium", -16))
        self.label_1.grid(row=1, column=0, pady=10, padx=10)

        self.button_0 = customtkinter.CTkButton(master=self.frame_left, text="BATCH", fg_color="maroon1", command=self.batch)
        self.button_0.grid(row=1, column=0, pady=10, padx=20)

        # OPEN the .lif / .tif / .tiff file
        self.combobox_time = customtkinter.CTkComboBox(master=self.frame_left, values=["Timepoint", "0", "1", "2", "3", "4", "5"])
        self.combobox_time.grid(row=2, column=0, pady=10, padx=20)

        self.combobox_channel = customtkinter.CTkComboBox(master=self.frame_left, values=["Channel", "0", "1", "2", "3", "4", "5"])
        self.combobox_channel.grid(row=3, column=0, pady=10, padx=20)

        self.combobox_nb_lif = customtkinter.CTkComboBox(
        ...
        self.combobox_segmentation = customtkinter.CTkComboBox(master=self.frame_left, values=["Segmentation", "on", "off"])
        self.combobox_segmentation.grid(row=5, column=0, pady=10, padx=20)

        # CHOOSE the file and SAVE the results
        self.button_1 = customtkinter.CTkButton(master=self.frame_left, text="Choose File", command=self.open_file)
        self.button_1.grid(row=6, column=0, pady=10, padx=20)

        self.button_2 = customtkinter.CTkButton(master=self.frame_left, text="Save parameters", command=self.save_parameters)
        self.button_2.grid(row=7, column=0, pady=10, padx=20)

        self.button_3 = customtkinter.CTkButton(master=self.frame_left, text="Save mesh CFD", command=self.save_mesh_CFD)
        ...
        self.combobox_color_group1 = customtkinter.CTkOptionMenu(
            master=self.frame_left,
            values=[
                "blue",
                "deepskyblue",
        ...
        self.combobox_color_group2 = customtkinter.CTkOptionMenu(
            master=self.frame_left,
            values=[
                "blue",
                "deepskyblue",
        ...
        self.combobox_color_group3 = customtkinter.CTkOptionMenu(
            master=self.frame_left,
            values=[
                "blue",
                "deepskyblue",
        ...
        self.combobox_color_group4 = customtkinter.CTkOptionMenu(
            master=self.frame_left,
            values=[
                "blue",
                "deepskyblue",
        ...
        self.combobox_color_group5 = customtkinter.CTkOptionMenu(
            master=self.frame_left,
            values=[
                "blue",
                "deepskyblue",
        ...
        self.combobox_color_group6 = customtkinter.CTkOptionMenu(
            master=self.frame_left,
            values=[
                "blue",
                "deepskyblue",
        ...
        self.plot_button = customtkinter.CTkButton(master=self.frame_left, text="Plot", command=self.button_plot)
        ...
        # configure grid layout of the frame_parameters (6x3)
        self.frame_parameters.rowconfigure((1, 2, 3), weight=1)
        ...
        self.label_info_1 = customtkinter.CTkLabel(
            master=self.frame_info, text="Image file", height=100, corner_radius=6, fg_color=("white", "gray38"), justify=tkinter.LEFT
        )
        self.label_info_1.grid(column=0, row=0, sticky="nwe", padx=15, pady=15)
        self.label_info_1.configure(font=("Verdana", 11, "italic"))

        # Progressbar, here inactive as no file in the system yet
        self.progressbar = customtkinter.CTkSlider(master=self.frame_info, from_=1, to=2, number_of_steps=2)
        ...
        self.button_analysis = customtkinter.CTkButton(master=self.frame_parameters, text="Vasculature analysis", command=self.print_parameters)
        self.button_analysis.grid(row=1, column=0, columnspan=7, pady=10, padx=20)

        self.label_diameter = customtkinter.CTkLabel(master=self.frame_parameters, text="Diameter [h,v]: ")
        self.label_diameter.grid(row=2, column=0, columnspan=2, pady=2, padx=5, sticky="ew")

        self.label_area = customtkinter.CTkLabel(master=self.frame_parameters, text="Area: ")
        self.label_area.grid(row=3, column=0, columnspan=2, pady=2, padx=5, sticky="ew")

        self.label_volume = customtkinter.CTkLabel(master=self.frame_parameters, text="Volume: ")
        self.label_volume.grid(row=2, column=2, columnspan=2, pady=2, padx=5, sticky="ew")

        self.label_volume_percentage_occupancy = customtkinter.CTkLabel(master=self.frame_parameters, text="V % occupancy: ")
        self.label_volume_percentage_occupancy.grid(row=3, column=2, columnspan=2, pady=2, padx=5, sticky="ew")

        self.label_average_branch_length = customtkinter.CTkLabel(master=self.frame_parameters, text="Branch length: ")
        self.label_average_branch_length.grid(row=2, column=4, columnspan=2, pady=2, padx=5, sticky="ew")

        self.label_number_branches_per_volume = customtkinter.CTkLabel(master=self.frame_parameters, text="Nb branches / V: ")
        ...
        self.combobox_1 = customtkinter.CTkComboBox(
            ...
            values=[
                "Branch distance by branch type - only for 1 file, blue color",
                "Number branches",
        ...
        self.button_group1 = customtkinter.CTkButton(master=self.frame_right, text="Group 1", fg_color="gray12", command=self.select_files_group1)
        ...
        self.button_group2 = customtkinter.CTkButton(master=self.frame_right, text="Group 2", fg_color="gray12", command=self.select_files_group2)
        ...
        self.button_group3 = customtkinter.CTkButton(master=self.frame_right, text="Group 3", fg_color="gray12", command=self.select_files_group3)
        ...
        self.button_group4 = customtkinter.CTkButton(master=self.frame_right, text="Group 4", fg_color="gray12", command=self.select_files_group4)
        ...
        self.button_group5 = customtkinter.CTkButton(master=self.frame_right, text="Group 5", fg_color="gray12", command=self.select_files_group5)
        ...
        self.button_group6 = customtkinter.CTkButton(master=self.frame_right, text="Group 6", fg_color="gray12", command=self.select_files_group6)
    ...
    def open_file(self):
        ...
        filename = filedialog.askopenfilename(
            title="Select A File", filetypes=(("all files", "*.*"), ("lif files", "*.lif"), ("tif files", "*.tif"), ("tiff files", "*.tiff"))
        )  # returns the name of the file, and the location of the file
        ...
        if timepoint == "Timepoint" or channel == "Channel" or nb == "Series" or segmentation == "Segmentation":
            self.message = messagebox.showinfo(
                title="Error",
                message="Please check your timepoint, series, channel and segmentation. \n"
                + "\n"
        ...
        # ============ IMAGE PROCESSING ============
            ...
            # ============ RENEW the buttons ============
            self.button_1 = customtkinter.CTkButton(master=self.frame_left, text="Choose Other", command=self.open_file)
            self.button_1.grid(row=6, column=0, pady=10, padx=20)
            # RENEW the save_parameters button to original state, as if the user had
            # been saving another file right before, it would be in the "SAVED" state
            self.button_2 = customtkinter.CTkButton(master=self.frame_left, text="Save parameters", command=self.save_parameters)
            self.button_2.grid(row=7, column=0, pady=10, padx=20)
            ...
            self.button_analysis = customtkinter.CTkButton(
                master=self.frame_parameters, text="Vasculature analysis", command=self.print_parameters  # RENEW the vasculature parameters button
            )
            ...
            self.label_volume_percentage_occupancy = customtkinter.CTkLabel(master=self.frame_parameters, text="% Occupancy: ")
            self.label_volume_percentage_occupancy.grid(row=3, column=2, columnspan=2, pady=2, padx=5, sticky="ew")
            ...
            self.button_group1 = customtkinter.CTkButton(
                master=self.frame_right, text="Group 1", fg_color="gray49", command=self.select_files_group1
            )  # RENEW the groups Buttons
            self.button_group1.grid(row=10, column=1, pady=10, padx=20, sticky="w")
            self.button_group2 = customtkinter.CTkButton(master=self.frame_right, text="Group 2", fg_color="gray49", command=self.select_files_group2)
            self.button_group2.grid(row=10, column=2, pady=10, padx=20, sticky="w")
            self.button_group3 = customtkinter.CTkButton(master=self.frame_right, text="Group 3", fg_color="gray49", command=self.select_files_group3)
            self.button_group3.grid(row=10, column=3, pady=10, padx=20, sticky="w")
            self.button_group4 = customtkinter.CTkButton(master=self.frame_right, text="Group 4", fg_color="gray49", command=self.select_files_group4)
            self.button_group4.grid(row=10, column=4, pady=10, padx=20, sticky="w")
            self.button_group5 = customtkinter.CTkButton(master=self.frame_right, text="Group 5", fg_color="gray49", command=self.select_files_group5)
            self.button_group5.grid(row=10, column=5, pady=10, padx=20, sticky="w")
            self.button_group6 = customtkinter.CTkButton(master=self.frame_right, text="Group 6", fg_color="gray49", command=self.select_files_group6)
            self.button_group6.grid(row=10, column=6, pady=10, padx=20, sticky="w")

            # Progressbar (appears when max-projection displayed)
            show_var = tkinter.DoubleVar()
            show_var.set(1)
            self.progressbar = customtkinter.CTkSlider(
                master=self.frame_right, from_=0, to=2, number_of_steps=2, variable=show_var, command=lambda event: self.slider_show(filename, show_var)
            )
            ...
            scale_x_px, scale_y_px, scale_z_px, scale_x, scale_y, scale_z, x_total, y_total, z_stacks, x_total_um, y_total_um, depth = metadata(
                filename, nb_lif
            )

            # Image processing
            # to blur just enough for a segmentation as accurate as possible
            kernel = 0.5
    ...
    def print_parameters(self):
        ...
        global branch_data, number_branches, number_isolated_branches, number_endpoint_junctions_branches
        global number_junction_junction_branches, number_circular_branches, number_junctions, number_endpoints
        global average_branch_length, total_length
        ...
        (
            ...
            endpoints,
            junctions,
        ) = skeleton_3d_analysis(skeleton_3d, x_total, y_total, scale_x)
        ...
        (
            ...
            volume_total_fibrin_gel,
            area_total_fibrin_gel,
            volume_percentage_occupancy,
            diameter_horizontal,
            diameter_vertical,
            mean_diameter_horizontal,
            mean_diameter_vertical,
        ) = area_volume_diameters(
            self, surf, voxels, image, mask_skel, scale_x, scale_y, scale_z, x_total_um, y_total_um, depth, coord_src_0, coord_dst_0, coord_src_1, coord_dst_1
        )
        ...
        # ============ DISPLAY the results ============
        self.button_analysis = customtkinter.CTkButton(master=self.frame_parameters, text="Vasculature analysis", state=tkinter.DISABLED)
        self.button_analysis.grid(row=1, column=0, columnspan=6, pady=10, padx=20)

        self.label_diameter = customtkinter.CTkLabel(
            master=self.frame_parameters, text="Diameter [h,v]: " + f"{mean_diameter_horizontal:.1E}" + " µm, " + f"{mean_diameter_vertical:.1E}" + " µm"
        )
        self.label_diameter.grid(row=2, column=0, columnspan=2, pady=2, padx=5, sticky="ew")

        self.label_area = customtkinter.CTkLabel(master=self.frame_parameters, text="Area: " + f"{area:.1E}" + " µm2")
        self.label_area.grid(row=3, column=0, columnspan=2, pady=2, padx=5, sticky="ew")

        self.label_volume = customtkinter.CTkLabel(master=self.frame_parameters, text="Volume: " + f"{volume:.1E}" + " µm3")
        self.label_volume.grid(row=2, column=2, columnspan=2, pady=2, padx=5, sticky="ew")

        self.label_volume_percentage_occupancy = customtkinter.CTkLabel(
            master=self.frame_parameters, text="% Occupancy: " + f"{round(volume_percentage_occupancy*100,1)}" + " %"
        )
        self.label_volume_percentage_occupancy.grid(row=3, column=2, columnspan=2, pady=2, padx=5, sticky="ew")

        self.label_average_branch_length = customtkinter.CTkLabel(master=self.frame_parameters, text="Branch length: " + f"{average_branch_length:.1E}" + " µm")
        self.label_average_branch_length.grid(row=2, column=4, columnspan=2, pady=2, padx=5, sticky="ew")

        self.label_number_branches_per_volume = customtkinter.CTkLabel(
            master=self.frame_parameters, text="Nb branches / V: " + f"{number_branches_per_volume:.1E}"
        )
        self.label_number_branches_per_volume.grid(row=3, column=4, columnspan=2, pady=2, padx=5, sticky="ew")
    ...
    def save_parameters(self):
        if process == 0:
            ...
            branch_data["metadata_id"] = pd.Series(
                [
                    "scale_x_px",
                    "scale_y_px",
                    "scale_z_px",
            ...
            branch_data["metadata"] = pd.Series(
                [scale_x_px, scale_y_px, scale_z_px, scale_x, scale_y, scale_z, x_total, y_total, z_stacks, x_total_um, y_total_um, depth]
            )
            ...
            # save x, y, z of the skeleton
            file_name_for_xcl_skel = (
                Path(filename).stem + "_data_analysis series n°" + str(nb_lif + 1) + " t=" + str(time) + " c=" + str(channel_fluo) + " skeleton x, y ,z"
            )
            ...
            if xcl_directory:
                ...
                self.button_2 = customtkinter.CTkButton(master=self.frame_left, text="SAVED", state=tkinter.DISABLED)
                self.button_2.grid(row=7, column=0, pady=10, padx=20)
    ...
    def save_mesh_CFD(self):
        ...
        if stl_directory:
            ...
            # µm/px
            vol_CFD = vedo.Volume(mask_transposed_CFD, spacing=(scale_x, scale_y, scale_z))
        ...
        # ============ RENEW the button ============
        self.button_3 = customtkinter.CTkButton(master=self.frame_left, text="SAVED", state=tkinter.DISABLED)
        self.button_3.grid(row=8, column=0, pady=10, padx=20)
    ...
    def select_files_group1(self):
        ...
        filenames_group1 = filedialog.askopenfilenames(title="Open files", initialdir="/", filetypes=filetypes)
        files.append(filenames_group1)
        # Choose files = group 1
        self.button_group1 = customtkinter.CTkButton(master=self.frame_right, text="Group 1", fg_color="turquoise1", command=self.select_files_group1)
        self.button_group1.grid(row=10, column=1, pady=10, padx=20, sticky="w")

    def select_files_group2(self):
        global filenames_group2
        filetypes = (("Excel files", "*.xlsx"), ("All files", "*.*"))
        filenames_group2 = filedialog.askopenfilenames(title="Open files", initialdir="/", filetypes=filetypes)
        files.append(filenames_group2)
        # Choose files = group 2
        self.button_group2 = customtkinter.CTkButton(master=self.frame_right, text="Group 2", fg_color="turquoise1", command=self.select_files_group2)
        self.button_group2.grid(row=10, column=2, pady=10, padx=20, sticky="w")

    def select_files_group3(self):
        global filenames_group3
        filetypes = (("Excel files", "*.xlsx"), ("All files", "*.*"))
        filenames_group3 = filedialog.askopenfilenames(title="Open files", initialdir="/", filetypes=filetypes)
        files.append(filenames_group3)
        # Choose files = group 3
        self.button_group3 = customtkinter.CTkButton(master=self.frame_right, text="Group 3", fg_color="turquoise1", command=self.select_files_group3)
        self.button_group3.grid(row=10, column=3, pady=10, padx=20, sticky="w")

    def select_files_group4(self):
        global filenames_group4
        filetypes = (("Excel files", "*.xlsx"), ("All files", "*.*"))
        filenames_group4 = filedialog.askopenfilenames(title="Open files", initialdir="/", filetypes=filetypes)
        files.append(filenames_group4)
        # Choose files = group 4
        self.button_group4 = customtkinter.CTkButton(master=self.frame_right, text="Group 4", fg_color="turquoise1", command=self.select_files_group4)
        self.button_group4.grid(row=10, column=4, pady=10, padx=20, sticky="w")

    def select_files_group5(self):
        global filenames_group5
        filetypes = (("Excel files", "*.xlsx"), ("All files", "*.*"))
        filenames_group5 = filedialog.askopenfilenames(title="Open files", initialdir="/", filetypes=filetypes)
        files.append(filenames_group5)
        # Choose files = group 5
        self.button_group5 = customtkinter.CTkButton(master=self.frame_right, text="Group 5", fg_color="turquoise1", command=self.select_files_group5)
        self.button_group5.grid(row=10, column=5, pady=10, padx=20, sticky="w")

    def select_files_group6(self):
        global filenames_group6
        filetypes = (("Excel files", "*.xlsx"), ("All files", "*.*"))
        filenames_group6 = filedialog.askopenfilenames(title="Open files", initialdir="/", filetypes=filetypes)
        files.append(filenames_group6)
        # Choose files = group 6
        self.button_group6 = customtkinter.CTkButton(master=self.frame_right, text="Group 6", fg_color="turquoise1", command=self.select_files_group6)
        self.button_group6.grid(row=10, column=6, pady=10, padx=20, sticky="w")
    ...
    def check_assumptions(self, data_for_statistics):
        ...
        # HOMOSCEDASTICITY
        # usually, samples are small. We thus use Levene's test. Note the use of * in the call that unpacks the list,
        # which is the same as calling with multiple argument
        res, p_var = levene(*data_for_statistics)
        if p_var > 0.05:
    ...
    def statistics(self, group_name, data_for_statistics, data_parameter, check_normality, check_homoscedasticity):
        ...
        if len(data_for_statistics) == 2:  # t-test between 2 independent groups
            ...
            if check_normality is True and check_homoscedasticity is True:
                ...
                plt.text((x_list[0] + x_list[1]) * 0.5, y + h, label_stat, ha="center", va="bottom", color=col)
                p_value = f"{p_value:.1E}"

            # NON PARAMETRIC test
            else:
                statistics_name = "Mann-Whitney U test"
                statistic, p_value = mannwhitneyu(data_for_statistics[0], data_for_statistics[1])
                # to cross-check the statistic: https://planetcalc.com/7858/
                if 0.01 < p_value < 0.05:
                    label_stat = "*"
                elif 0.001 < p_value < 0.01:
                    label_stat = "**"
        ...
        # ========= More than 2 groups =========
            ...
            if check_normality is True and check_homoscedasticity is True:
                ...
                if pvalue < 0.05:
                    ...
                    for i in range(len(tukey.pvalue)):
                        # tukey needs normality and homoscedasticity
                        p_list_i = tukey.pvalue[i]
                        ...
                        for j in range(len(p_list_i)):
                            ...
                            if 0.01 < pval < 0.05:
                                label_stat = "*"
                                # determine the height of the annotations
                                y, h, col = (data_parameter["Data"].max() + 30 + k, 10, "k")
                                # i first group, j second group
                                plt.plot([i, j], [y + h, y + h], lw=1.5, c=col)
                                plt.text((i + j) * 0.5, y + h, label_stat, ha="center", va="bottom", color=col)
                                k += 20  # for plotting
            ...
            # NON PARAMETRIC test
                ...
                # pvalue from the kruskal-wallis test
                statistic, pvalue = kruskal(*data_for_statistics)
                label_stat = []
                p_value = []  # list of all the p_values from the dunn test
                k = 0
                if pvalue < 0.05:  # post-hoc test
                    # as Kruskal-Wallis test above, we use a multiple comparison procedure on ranks: Dunn's test.
                    #  To avoid multiple-testing bias, we perform a bonferroni correction.
                    dunn = sp.posthoc_dunn(data_for_statistics, p_adjust="bonferroni")
                    for i in range(len(dunn)):
                        # as repeats in the dataframe, following the diagonal
                        for j in range(i, len(dunn)):
                            # p_value for the comparison between groups i and j; dunn dataframe, ie use .iat
                            pval = dunn.iat[i, j]
                            p_value.append(f"{pval:.1E}")
                            if 0.01 < pval < 0.05:
                                label_stat = "*"
    ...
    def button_plot(self):
        ...
        # ============ PLOTTING for the other parameters ============
            ...
            parameter == "Number branches"
            or parameter == "Number isolated branches"
            or parameter == "Number endpoint-junction branches"
            or parameter == "Number junction-junction branches"
            or parameter == "Number circular branches"
            or parameter == "Number junctions"
            or parameter == "Number endpoints"
            or parameter == "Average branch length"
            or parameter == "Total length"
            or parameter == "Junctions per volume"
            or parameter == "Branches per volume"
            or parameter == "Area"
            or parameter == "Volume"
            or parameter == "V percentage occupancy"
            or parameter == "Diameter_mean_horizontal"
            or parameter == "Diameter_mean_vertical"
            or parameter == "Diameter_horizontal"
            or parameter == "Diameter_vertical"
        ):

            # Get the groups' colors and names
            color_palette = [
                self.combobox_color_group1_var.get(),
                self.combobox_color_group2_var.get(),
                self.combobox_color_group3_var.get(),
                self.combobox_color_group4_var.get(),
                self.combobox_color_group5_var.get(),
                self.combobox_color_group6_var.get(),
            ]
            ...
            for group_files in files:  # files retrieved from select_files_group_i
                # full_data = data from the sheets grouped together in a "group"
                full_data, legend = self.read_excel(group_files, parameter)
                k = len(full_data)
                group = group_name[i]
                # name of the group, repeated the nb of times there is a data in full_data
                #  for adding in the dataframe
                group_list = [group] * k
            ...
            # final datasheet from which we'll plot
            data_parameter = pd.DataFrame(data_parameter_transient)

            # ================ Statistical assumptions ===============================
            # 1st parameter Title bar, 2nd one is the message we want to show in the popup
            response = messagebox.askokcancel("Statistical assumptions", "Please check hereafter the statistical assumptions for your data")
            if response:
        ...
        self.plot_button = customtkinter.CTkButton(master=self.frame_left, text="Plot", command=self.button_plot, fg_color="turquoise1")
        self.plot_button.grid(row=15, column=0, padx=2, pady=20, sticky="we")
    ...
    def change_appearance_mode(self, new_appearance_mode):
        ...
        if new_appearance_mode == "Light":
            self.button_group1 = customtkinter.CTkButton(master=self.frame_right, text="Group 1", fg_color="gray49", command=self.select_files_group1)
            self.button_group1.grid(row=10, column=1, pady=10, padx=20, sticky="w")
            # Choose files = group 2
            self.button_group2 = customtkinter.CTkButton(master=self.frame_right, text="Group 2", fg_color="gray49", command=self.select_files_group2)
            self.button_group2.grid(row=10, column=2, pady=10, padx=20, sticky="w")
